...
func Insert_wechat_chat_log(wechat_chat_log Wechat_chat_log) bool {
	//准备sql语句
	insert_sql := fmt.Sprintf("INSERT INTO `wechat_db`.`wechat_chat_log`(`time`, `send_user`, `send_content`, `send_group`) VALUES (\"%s\", \"%s\", \"%s\", \"%s\");", wechat_chat_log.Time, wechat_chat_log.Send_user, wechat_chat_log.Send_content, wechat_chat_log.Send_group)
